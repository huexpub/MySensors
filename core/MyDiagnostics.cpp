...
void diagnosticsClearMySensorsTransportSettings(void)
{
	...
	MY_SERIALDEVICE.println(F("> TSP ID CFG CLR"));
}

void diagnosticsClearMySensorsFWConfig(void)
{

	uint8_t buffer[8 /* 4x uint16_t, see nodeFirmwareConfig_t*/];
	for (uint8_t i = 0; i < sizeof(buffer); i++) {
		buffer[i] = 0xFF;
	}
	hwWriteConfigBlock((void *)&buffer, (void *)EEPROM_FIRMWARE_TYPE_ADDRESS,
	                   sizeof(buffer));
	MY_SERIALDEVICE.println(F("> FW CFG CLR"));
}
...
void diagnosticsEEPROMMenu(void)
{
	while (true) {
		...
		if (inputCmd == 'D') {
			...
			diagnosticsClearMySensorsFWConfig();
		} else if (inputCmd == 'X') {
			return;
		}
	}
