...
#error Multitransport requires the new RFM69 driver!
#endif

#if (MY_TRANSPORT_COUNT <= 3)
// 2 - 3 transports, 2 bits: 0=BC, 1=TSP1, 2=TSP2, 3=TSP3
// 256 / 4 = 64 bytes RAM
#define SIZE_BITS_PER_CHANNEL 2
#elif (MY_TRANSPORT_COUNT > 3) && (MY_TRANSPORT_COUNT <= 15)
// 4 - 7 transports, 4 bits: 0000 - 1111
// 256 / 2 = 128 bytes RAM
#define SIZE_BITS_PER_CHANNEL 4
#else
#error >15 transports not supported :)
#endif
...
bool transportHALInit(void)
{
	...
	RFM69_transportEncrypt(transportPSK);
#else
	//set up AES-key
