...
#if defined(MY_RF24_ATC)
uint8_t RF24_TX_POWER[256];
#endif
...
#if defined(MY_RF24_ATC)
	RF24_setTxPowerLevel(RF24_TX_POWER[to]);
	Serial.print("SET RF24 power = ");
	Serial.println(RF24_TX_POWER[to]);
#endif
	const bool result = RF24_sendMessage(to, tx_data, finalLength, noACK);
#if defined(MY_RF24_ATC)
	// do not adjust for broadcasting
	if (!noACK) {
		const uint8_t TXval = RF24_getObserveTX() & 0xF;
		Serial.print("RF24 OBS =");
		Serial.println(TXval);
		uint8_t TXpower = RF24_TX_POWER[to];
		if ((TXval > 0) && (TXpower < 3)) {
			TXpower++;
			Serial.println("INC RF24");
		} else if ((TXval == 0) && (TXpower > 0)) {
			TXpower--;
			Serial.println("DEC RF24");
		}
		RF24_TX_POWER[to] = TXpower;
	}
#endif
	return result;
}
...
{
#if defined(MY_RF24_ATC)
	for (uint16_t i = 0; i < sizeof(RF24_TX_POWER); i++) {
		RF24_TX_POWER[i] = MY_RF24_PA_LEVEL;
	}
#endif
