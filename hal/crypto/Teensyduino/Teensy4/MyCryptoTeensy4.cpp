...
void DCP_HASH_Init(dcp_handle_t *handle, dcp_hash_ctx_t *ctx, dcp_hash_algo_t algo)
{
	...
	for (size_t i = 0; i < sizeof(ctxInternal->blk.w) / sizeof(ctxInternal->blk.w[0]); i++) {
    ...
    const uint8_t *iv)
{
	...
	dcpPacket->control0 =
	    0x322u | (handle->swapConfig &
	              0xFC0000u); /* CIPHER_INIT | CIPHER_ENCRYPT | ENABLE_CIPHER | DECR_SEMAPHORE */
    ...
    const uint8_t *iv)
{
	...
	dcpPacket->control0 = 0x222u | (handle->swapConfig &
	                                0xFC0000u); /* CIPHER_INIT | ENABLE_CIPHER | DECR_SEMAPHORE */
	dcpPacket->control1 = 0x10u;                                     /* CBC */
                ...
                size_t dataLength)
{
	...
	handle.channel = kDCP_Channel0;
	handle.swapConfig = kDCP_NoSwap;
	handle.keySlot = kDCP_PayloadKey;

	DCP_AES_SetKey(&handle, key, 16 /*keyLength*/); // 16 bytes
	DCP_HASH_Init(&handle, &ctx, kDCP_Sha256);
	DCP_HASH_Update(&ctx, data, dataLength);
	DCP_HASH_Finish(&ctx, dest);
}

dcp_handle_t aes_handle;

void AES128CBCInit(const uint8_t *key)
{
	aes_handle.channel = kDCP_Channel0;
	aes_handle.swapConfig = kDCP_NoSwap;
	aes_handle.keySlot = kDCP_KeySlot0;    // could use OTP key

	DCP_AES_SetKey(&aes_handle, key, 16);
}

void AES128CBCEncrypt(uint8_t *iv, uint8_t *buffer, const size_t dataLength)
{
	DCP_AES_EncryptCbc(&aes_handle, buffer, buffer, dataLength, iv);
}

void AES128CBCDecrypt(uint8_t *iv, uint8_t *buffer, const size_t dataLength)
{
	DCP_AES_DecryptCbc(&aes_handle, buffer, buffer, dataLength, iv);
}
...
void SHA256(uint8_t *dest, const uint8_t *data, size_t dataLength)
{
	...
	DCP_HASH_Init(&m_handle, &hashCtx, kDCP_Sha256);
	DCP_HASH_Update(&hashCtx, data, dataLength);
	DCP_HASH_Finish(&hashCtx, dest);
}
